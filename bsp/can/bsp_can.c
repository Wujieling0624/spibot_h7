...
void CANServiceInit()
{
	...
	// HAL_FDCAN_ConfigGlobalFilter(&hfdcan1, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_FILTER_REMOTE, FDCAN_FILTER_REMOTE);//全局过滤器设置
    // HAL_FDCAN_ActivateNotification(&hfdcan1, FDCAN_IT_RX_FIFO0_NEW_MESSAGE | FDCAN_IT_TX_FIFO_EMPTY, 0);
...
CANInstance *CANRegister(CAN_Init_Config_s *config)
{
	...
	instance->txconf.BitRateSwitch = FDCAN_BRS_ON;         	// 兼容CAN2.0禁用位速率切换
	if(config->can_mode & 0x01 == 1)
	{
		instance->txconf.FDFormat = FDCAN_FD_CAN;			// 使用FD CAN
	}
	else
	{
		instance->txconf.FDFormat = FDCAN_CLASSIC_CAN;		// 使用标准CAN
	
	}
	uint8_t a = config->can_mode & 0x02;
	if(a == 2)
	{
		instance->txconf.IdType = FDCAN_EXTENDED_ID;		// 使用扩展ID
	}
	else
	{
...
static void FDCANFIFOxCallback(FDCAN_HandleTypeDef *_hfdcan, uint32_t fifox)
{
    ...
    static uint8_t fdcan_rx_buff[64];
